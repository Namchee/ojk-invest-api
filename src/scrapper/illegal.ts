 ...
 */
export class IllegalScrapper extends Scrapper<IllegalInvestment> {
  // row selector
  ...
  protected async scrapPage(
    ...
    await page.waitForSelector(IllegalScrapper.rowSelector);

    const rawIllegals = await page.$$eval(
      IllegalScrapper.rowSelector,
      (rows) => {
  ...
  public async scrapInfo(): Promise<void> {
    ...
    await page.waitForSelector(IllegalScrapper.pageSelector);
      ...
      if (investments.length === 0 ||
        ...
        const isLastPage = await page.$$eval(
          ...
          (buttons, selector) => {
            ...
            return isDisabled;
          }, IllegalScrapper.disabledSelector);
